...
script.on_init(function()
	storage = {
		...
		repeating_beacons = {},
		offline_beacons = {},
	} -- TODO: refactor/migrate "strict_beacons" as "types" instead
	initialize()
	startup()
	check_global_list()
end)
...
function startup()
	...
	if active_scripts then
		enable_scripts(script.active_mods)
	end
end
...
function verify_technology_unlocks()
	local techs = {
		"effect-transmission",
		"ab-novel-effect-transmission",
		"ab-medium-effect-transmission",
		"ab-long-effect-transmission",
		"ab-focused-beacon",
		"ab-node-beacon",
		"ab-conflux-beacon",
		"ab-hub-beacon",
			...
			if force.technologies[tech] and force.technologies[tech].researched then
				for _, effect in pairs(force.technologies[tech].prototype.effects) do
					if
						...
						and force.recipes[effect.recipe]
						and not force.recipes[effect.recipe].enabled
					then
...
function enable_scripts(mods)
	...
	if update_rate > 0 then
		register_periodic_updates(update_rate * 60)
	end
	if persistent_alerts == true then
	...
	if remote.interfaces["PickerDollies"] and remote.interfaces["PickerDollies"]["dolly_moved_entity_id"] then
		script.on_event(remote.call("PickerDollies", "dolly_moved_entity_id"), function(event)
			check_remote(event["moved_entity"], "added", 1)
		end) -- rechecks nearby beacons when a beacon is moved; a maximum moved distance of 1 is assumed
	end
	if mods["pycoalprocessing"] then
		remote.remove_interface("cryogenic-distillation")
		remote.add_interface("cryogenic-distillation", {
			am_fm_beacon_settings_changed = function(new_beacon)
				check_remote(new_beacon, "added", 0)
			end, -- rechecks nearby beacons when an AM:FM beacon is updated
			am_fm_beacon_destroyed = function(receivers, surface) end, -- unused
		})
	end
	if mods["informatron"] then
		remote.remove_interface("alternative-beacons-ex")
		remote.add_interface("alternative-beacons-ex", {
			informatron_menu = function(data)
				return informatron_beacon_menu(data.player_index)
			end,
			informatron_page_content = function(data)
				return informatron_beacon_page_content(data.page_name, data.player_index, data.element)
			end,
		})
	end
end
...
function on_settings_changed(event)
	if event.setting == "ab-update-rate" then
		...
		if previous_update_rate ~= update_rate then
			if previous_update_rate > 0 then
...
function register_periodic_updates(tick_rate)
	script.on_nth_tick(tick_rate, function(event)
		check_global_list()
	end)
end
...
function register_alert_refreshing()
	script.on_nth_tick(601, function(event)
		refresh_beacon_alerts()
	end)
end
...
function informatron_beacon_menu(player_index)
	return {}
end

function informatron_beacon_page_content(page_name, player_index, element)
	if page_name == "alternative-beacons-ex" then
		element.add({ type = "label", name = "text_1", caption = {
			"alternative-beacons-ex.page_alternative-beacons_text_1",
		} })
		...
		element.add({ type = "label", name = "text_2", caption = {
			"alternative-beacons-ex.page_alternative-beacons_text_2",
		} })
	end
end
...
function check_nearby(entity, behavior)
	...
	if storage.strict_beacons[entity.name].strict ~= nil then
	...
	if storage.strict_beacons[entity.name].hub == nil and entity.name ~= "ll-oxygen-diffuser" then
		for _, nearby_entity in pairs(nearby_entities) do
			if storage.strict_beacons[nearby_entity.name].hub then
				if
					get_distance(entity.selection_box, nearby_entity.selection_box)
					< storage.exclusion_ranges[nearby_entity.name]
				then
					hubCount = hubCount + 1
	...
	for _, nearby_entity in pairs(nearby_entities) do
		if nearby_entity.unit_number ~= entity.unit_number and nearby_entity.name ~= "ll-oxygen-diffuser" then
			...
			if hubCount > 0 then
				if check_influence(nearby_entity, hubIDs) then
					exclusion_mode = "super"
				end -- beacons within a single hub's area affect each other differently
			end
			if exclusion_mode == "strict" then
				disabling_range =
					math.max(disabling_range, exclusion_range + storage.distribution_ranges[nearby_entity.name])
			end
			if exclusion_mode == "super" then
				disabling_range = math.max(
					disabling_range,
					math.max(exclusion_range, storage.distribution_ranges[entity.name])
						+ math.max(
							storage.exclusion_ranges[nearby_entity.name],
							storage.distribution_ranges[nearby_entity.name]
						)
				)
			end
			if storage.strict_beacons[nearby_entity.name].conflux then
				local nearby_distribution_width = 2 * storage.distribution_ranges[nearby_entity.name]
					+ nearby_entity.selection_box.right_bottom.x
					- nearby_entity.selection_box.left_top.x
				local distribution_width = 2 * storage.distribution_ranges[entity.name]
					+ entity.selection_box.right_bottom.x
					- entity.selection_box.left_top.x
				if distribution_width >= nearby_distribution_width then
					disabling_range = math.max(
						disabling_range,
						storage.distribution_ranges[entity.name] + storage.distribution_ranges[nearby_entity.name]
					)
				end -- semi-strict
			end
...
function check_self(entity, removed_id, wasEnabled)
		...
		for _, nearby_entity in pairs(nearby_entities) do
			if storage.strict_beacons[nearby_entity.name].hub and nearby_entity.unit_number ~= removed_id then
				...
				then
					hubCount = hubCount + 1
					if hubCount == 1 then
						hubID = nearby_entity.unit_number
					end
					table.insert(hubIDs, nearby_entity.unit_number)
					isEnabled = false
				end
	...
	for _, nearby_entity in pairs(nearby_entities) do
		if
			nearby_entity.unit_number ~= entity.unit_number
			and nearby_entity.unit_number ~= removed_id
			and nearby_entity.name ~= "ll-oxygen-diffuser"
		then
			local exclusion_mode = "normal"
			local nearby_distance = get_distance(entity.selection_box, nearby_entity.selection_box)
			local disabling_range = storage.exclusion_ranges[nearby_entity.name]
			if storage.strict_beacons[nearby_entity.name].strict ~= nil then
				exclusion_mode = "strict"
			end
			...
			if exclusion_mode == "strict" then
				disabling_range = math.max(
					disabling_range,
					storage.exclusion_ranges[nearby_entity.name] + storage.distribution_ranges[entity.name]
				)
			end
			if exclusion_mode == "super" then
				disabling_range = math.max(
					...
					math.max(
						...
						+ math.max(storage.exclusion_ranges[entity.name], storage.distribution_ranges[entity.name])
				)
			end
			if storage.strict_beacons[entity.name].conflux then
				...
				local distribution_width = 2 * storage.distribution_ranges[entity.name]
					+ entity.selection_box.right_bottom.x
					- entity.selection_box.left_top.x
				if nearby_distribution_width >= distribution_width then
					disabling_range = math.max(
						disabling_range,
						storage.distribution_ranges[entity.name] + storage.distribution_ranges[nearby_entity.name]
					)
			...
			if storage.strict_beacons[nearby_entity.name].hub == nil then
				if nearby_distance < disabling_range then
					if exclusion_mode == "super" or use_repeating_behavior(nearby_entity, entity) == false then
						isEnabled = false
					end -- some beacons don't affect each other
				end
			elseif storage.strict_beacons[nearby_entity.name].hub and storage.strict_beacons[entity.name].conflux then
				disabling_range = storage.distribution_ranges[entity.name]
					+ storage.distribution_ranges[nearby_entity.name]
				if nearby_distance < disabling_range then
					isEnabled = false
				end
	...
	if isEnabled == false and (entity.name == "ei_copper-beacon" or entity.name == "ei_iron-beacon") then
		isEnabled = true
	end
	if
		entity.name == "base-power-crystal-1"
		or entity.name == "base-power-crystal-2"
		or entity.name == "base-power-crystal-3"
		or entity.name == "base-power-crystal-negative-1"
		or entity.name == "base-power-crystal-negative-2"
	then
		isEnabled = true
	end
...
function remove_beacon_alert(beacon_entity)
	if beacon_entity.valid then
		for _, player in pairs(beacon_entity.force.players) do
			player.remove_alert({
				entity = beacon_entity,
				type = defines.alert_type.custom,
				position = beacon_entity.position,
				surface = beacon_entity.surface,
				message = { "description.ab_beacon_offline_alert" },
				icon = { type = "virtual", name = "ab_beacon_offline" },
			})
		end
...
function check_influence(entity, hubIDs)
	...
	if storage.exclusion_ranges["ab-hub-beacon"] ~= nil then
		...
		local exclusion_area = {
			...
			{
				entity.selection_box.right_bottom.x + exclusion_range,
				entity.selection_box.right_bottom.y + exclusion_range,
			},
		}
		...
		for _, beacon in pairs(nearby_beacons) do
			if storage.strict_beacons[beacon.name].hub then
				for _, hub_number in pairs(hubIDs) do
					if beacon.unit_number == hub_number then
						isInfluenced = true
					end
				end
			end
...
function use_repeating_behavior(entity1, entity2)
	...
	if entity1.unit_number ~= entity2.unit_number then
		if storage.repeating_beacons[entity1.name] and storage.repeating_beacons[entity1.name][entity2.name] then
			result = true
		end
	end
	do
